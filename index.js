...
async function connectToWA() {
    ...
    // Load session if available (now handles both MONGODB and MEGA)
   ...
   // Send settings menu after connection message
    ...
    \`\`\`XMD BOT SETTINGS\`\`\`
    -----------------------------------------
  Use ${prefix}menu for all options
                    `;
...
app.get('/', (req, res) => {
  ...
  app.listen(port, () => console.log(`Xmd Server listening on port http://localhost:${port}`));
  setTimeout(() => {
  connectToWA()
  }, 4000);
